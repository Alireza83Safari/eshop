...
export default function EditAppPic({ setShowEditAppPic, editAppPicId }) {
  ...
  useEffect(() => {
    const fetchAppPic = async () => {
      setLoading(true);
      try {
        adminAxios.get(`/appPic/${editAppPicId}`).then((infos) => {
          setAppicInfo({
            ...appicInfo,
            appPicType: infos?.data?.appPicType,
            description: infos?.data?.description,
            priority: infos?.data?.priority,
        ...
        <form onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="flex justify-center mt-8">
            ...
            <button
              ...
              onClick={() => setShowEditAppPic(false)}
            >
              Cancel
            </button>
          </div>
